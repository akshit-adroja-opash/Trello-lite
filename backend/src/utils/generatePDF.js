...
export const generatePDF = async (data, fileName) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Create A4 PDF with set margins and buffered pages
            ...
            const execSummaryText = `This report provides a snapshot of the current development velocity for board hhk. As of the generation date, the project shows a significant accumulation of tasks in the Backlog phase. Initial triage is required to move items into active development to maintain operational milestones.`;
            ...
            for (let i = range.start; i < range.start + range.count; i++) {
                ...
                if (i > range.start) {
                    ...
                    doc.font("Helvetica")
                       .text(`BOARD: ${data.boardName.toUpperCase()}`, 40, 18, { align: "right", width: doc.page.width - 80 });
                    doc.restore();
                }

                // Draw Footer bottom panel on all pages
                doc.save();
                ...
                const cleanBoardName = data.boardName.toUpperCase().replace(/[^A-Z0-9]/g, '');
