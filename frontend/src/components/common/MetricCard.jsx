  ...
  return (
    <div className={`bg-surface-container-lowest dark:bg-slate-800 rounded-xl p-lg shadow-sm border flex flex-col justify-between gap-md
      ...
      <div className="flex justify-between items-start">
        <span className={`font-label-caps text-label-caps capitalize tracking-wider ${danger ? 'text-error' : 'text-on-surface-variant dark:text-slate-400'}`}>{label}</span>
        <span className={`material-symbols-outlined p-xs rounded-md text-[20px] ${danger ? 'text-error bg-error/10' : 'text-secondary dark:text-blue-400 bg-secondary/10 dark:bg-blue-900/30'}`}>{icon}</span>
      </div>
